...
public class Boss : MonoBehaviour
{
    ...
    public void Attack(int reduceGold, int reduceDiamond)
    {
        ...
        if (isAttack)
        {
            ...
            float speed = 1 / Vector3.Distance(transform.position, playerTransform.position);
            transform.position = Vector3.Lerp(transform.position, playerTransform.position, speed * Time.deltaTime * moveSpeed);
            //
    ...
    #region 辅助2
    /// <summary>一圈金币</summary>
    void GoldPrize_Bloom(int index)
    {
        GameObject itemGo = Instantiate(gold, transform.position, Quaternion.Euler(transform.eulerAngles + new Vector3(0, 18 + 36 * (index - 1), 0)));
        itemGo.GetComponent<Gold>().bossPrize = true;
    }
    /// <summary>一圈钻石</summary>
    void DiamandsPrize_Bloom(int index)
    {
        GameObject itemGo = Instantiate(diamands, transform.position, Quaternion.Euler(transform.eulerAngles + new Vector3(0, 36 + 36 * (index - 1), 0)));
        itemGo.GetComponent<Gold>().bossPrize = true;
    }
    #endregion
}
